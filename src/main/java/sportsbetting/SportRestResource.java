...
public class SportRestResource {
	...
	private SportService getAdminServiceFromSession(HttpSession session) throws SportException {
		SportService adminService = (SportService) session.getAttribute("admin");
		return adminService;
	}

	private SportService getBookieServiceFromSession(HttpSession session) throws SportException {
		SportService bookieService = (SportService) session.getAttribute("bookie");
		return bookieService;
	}

	private SportService getPlayerServiceFromSession(HttpSession session) throws SportException {
	...
	public Bet[] getBookieBets(HttpSession session) throws SportException {
		Bet[] bookieBets = getBookieServiceFromSession(session).getBookieBets().toArray(new Bet[0]);
		return bookieBets;
	}
	...
	public Bet[] getPlayerBets(HttpSession session) throws SportException {
		Bet[] playerBets = getPlayerServiceFromSession(session).getPlayerBets().toArray(new Bet[0]);
		return playerBets;
	}
	...
	public Message adminLogin(@PathVariable("userName") String userName, @PathVariable("password") String password,
		...
		if (sportService.adminLogin(ClientType.ADMINSPORT, userName, password) != null && userName != null
	...
	public Message bookieUserLogin(@PathVariable("userName") String userName, @PathVariable("password") String password,
		...
		if ((sportService.bookieLogin(ClientType.BOOKIE, userName, password)) != null && userName != null
				&& !userName.trim().isEmpty() && password != null && !password.trim().isEmpty()) {
	...
	public Message playerUserLogin(@PathVariable("userName") String userName, @PathVariable("password") String password,
		...
		if ((sportService.playerLogin(ClientType.PLAYER, userName, password)) != null && userName != null
				&& !userName.trim().isEmpty() && password != null && !password.trim().isEmpty()) {
