...
//  toString method for casting objects to Strings
...
// global service function for showing/hiding controllers in application
		...
		// ====== LOGIN CTRL will be shown before accessing clients page
		.controller(
				'loginCtrl',
				function($http, $scope, $rootScope, clientService) {
					...
					// init function for loading the login ctrl
					$scope.init = function() {
					...
					// login method for 3 types of clients
					...
					// ===== PLAYER CTRL will be shown after Player is logedIn
					.controller(
							'playerCtrl',
							function($http, $scope, $rootScope, clientService) {
								...
								// init function for loading the player ctrl
								$scope.init = function() {
									$scope.sport = "ALL";
									$scope.bet = null;
									$scope.maxWager = 0;
...
//					$scope.bookieBets = [];
								...
								.then(
										function(response) {
											...
											if (angular.isArray(response.data)) {
												...
												$scope.bookieBets = response.data;
											} else {
												console
														.log("got single bet, clearing the old bookieBets array, and get response to bookieBets[0]");
												$scope.bookieBets = [];
												$scope.bookieBets[0] = response.data;
											}
											console
					...
					$scope.removeBet = function(bet) {
						var url = clientService.restURL
								+ "bookie/removebet/" + $scope.bet.betId;
						$http({
							method : "DELETE",
							url : url,
							data : $scope.bet,
...
//					update Bookie method
					...
					$scope.removePlayer = function(player) {
						var url = clientService.restURL + "admin/players/" + $scope.player.playerId;
						$http({
							method : "DELETE",
							url : url,
							data : $scope.player,
